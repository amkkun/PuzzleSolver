...
solve :: Board -> [Board]
solve board
  | isNothing maybePos = [board]
  | otherwise = boardList board (fromJust maybePos) >>= solve
  where
    maybePos = lookup 0 $ map swap board

boardList :: Board -> Pos -> [Board]
boardList board pos = update board pos <$> filter (check board pos) [1..boardSize]
